...
LOG_FOLDERS = [
    r"C:\AirportDummy\Logs",                       # <-- adjust if you put logs elsewhere
    os.path.expandvars(r"%PROGRAMDATA%\AirportDummy\Logs"),
    os.path.expandvars(r"%LOCALAPPDATA%\AirportDummy\Logs"),
    os.path.expanduser(r"~\Documents\AirportDummy\Logs")
]
...
def is_stalled(proc, log_path, idle_seconds=60):
    try:
        ...
        if not log_path:
            return True
        return (time.time() - os.path.getmtime(log_path)) > idle_seconds
...
def build_report(exe, exit_code, log_snippet):
    ...
    blob = {"exe": exe, "exit_code": exit_code, "logs": log_snippet or ""}
